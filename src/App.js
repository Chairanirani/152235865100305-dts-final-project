...
function App() {
  ...
  return (
    <div className="App" style={{ backgroundImage: 'url("../assets/images/back-home.jpg")', backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}> 
      <Navbar userName={name} userEmail={user?.email} />
      <Box sx={{ flexGrow: 1, padding: '100px' }}>
        <Item>
          <Typography>WAKTU SHOLAT</Typography>
        </Item>
        <Item>
          <Autocomplete
            disablePortal
            ...
            options={top100Films}
            sx={{ width: 300 }}
            renderInput={(params) => <TextField {...params} label="Kota" onClick= {(e) => e.target.value? (setCity(e.target.value)) : (setCity('0')) }/>}
